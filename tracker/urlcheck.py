    ...
    def thread():
        while url_list:
            ...
            try:
                ...
                Image.open(BytesIO(cont))
                print(threading.current_thread().name + ': both pic and thumb URLs live for post in ' + post.location_text)

            # If the Instagram picture link is dead, deletes post so that it can be refreshed
            except:
                print(threading.current_thread().name + ': dead url, deleting post and post record for id: ' + str(post.post_id))
                post.delete()
                try:
